...
// will generate a random owner name
func RandomOwner() string {
	return RandomString(6) // we use six characters to reasonably avoid duplicates
}

// will generate a random amount of money
func RandomMoney() int64 {
	return RandomInt(0, 1000) // arbitrary min and max
}

// will generate a random currency code
func RandomCurrency() string {
	currencies := []string{"EUR", "USD", "CAD"} // arbitrary currency codes
	n := len(currencies)
